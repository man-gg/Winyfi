...
set PORT=3306
set DBUSER=winyfi
set DBPASS=YourStrongPassword!
set DBNAME=winyfi
set LOG_FILE=%~dp0mysql_check_error.log
...
"%MYSQL_BIN%" -h %HOST% -P %PORT% -u %DBUSER% -p%DBPASS% -e "SELECT 1" > nul 2> "%TEMP%\mysql_error.tmp"
...
"%MYSQL_BIN%" -h %HOST% -P %PORT% -u %DBUSER% -p%DBPASS% -e "USE %DBNAME%; SELECT COUNT(*) as table_count FROM information_schema.tables WHERE table_schema='%DBNAME%';" > nul 2> "%TEMP%\mysql_db_error.tmp"
